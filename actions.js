	...
	if (action.action === 'changeNdiSource') {
		if (action.options.source == 'custom') {
			...
			if (action.options.source) {
				(async () => {
					var sourceSize = await this.ndi.getDb().get('source').filter({ name: action.options.source }).size().value();
					if (sourceSize > 0) {
						var sourceDb = await this.ndi.getDb().get('source').filter({ name: action.options.source }).value();
		...
		if (action.options.source) {
			(async () => {
				var sourceSize = await this.ndi.getDb().get('source').filter({ name: action.options.source }).size().value();
				if (sourceSize > 0) {
					await this.ndi.getDb().get('source').remove({ name: action.options.source }).write();

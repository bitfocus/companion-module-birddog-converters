			...
			this.timer = setIntervalAsync(
				async () => {
						try {
							...
							for (let i = 0; i < this.source.length; i++) {
								var sourceDb = await db.get('source').filter({ name: this.source[i].name }).value();
								if (sourceDb.length > 0) {
									if (sourceDb.urlAddress != this.source[i].urlAddress) {
										db.get('source').find({ name: this.source[i].name }).assign({ urlAddress: this.source[i].urlAddress }).write();
									}
								} else {
